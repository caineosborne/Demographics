...
def force_process_candidate(candidate_id: int) -> dict[str, Any]:
    """Re-run one audited candidate after an explicit operator override."""
    candidate = research_store.get_candidate(int(candidate_id))
    url = str(candidate.get('url') or '').strip()
    if not url:
        raise ValueError('Candidate has no source URL to force process.')
    details = candidate.get('details') or {}
    scope_iso3 = str(details.get('country_iso3') or '').upper() or None
    settings = SearchSettings(
        categories=[], reddit_enabled=False, max_candidates=1,
        review_criteria=CRITERIA,
        country_hunt_mode='direct',
        country_hunt_iso3s=[scope_iso3] if scope_iso3 else [],
        forced_urls=[url],
    )
    result = _start_research(
        settings.model_dump(), queue_iso3s=[scope_iso3] if scope_iso3 else [],
        persist_settings=False,
    )
    result.update({'forced_candidate_id': int(candidate_id), 'forced_url': url})
    return result
